...
<c:set var="_page"
	value="${empty page ? requestScope[param.pageName] : page}" />
<c:choose>
...
<div class="page">
	...
	<c:forEach items="${_page.pageIndex}" var="pi" varStatus="pagevs">
		...
		<c:if test="${pi ne _page.currentPage}">
			<a href="javascript:subPage('${pi}')">${pi}</a>
		</c:if>
	</c:forEach>
	...
	前往第 <input type="text" class="input-mini" style="width: 40px;"
		id="pageidInput"
		onkeydown="if(event.keyCode == 13){inputpage();return false;}"
		size="3" /> 页&nbsp; <input type="button" value="go"
		class="go btn btn-info btn-small" onclick="inputpage();return false;" />
</div>